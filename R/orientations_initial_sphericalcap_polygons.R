...
#'   if FALSE, only those for the bottom (`beta1`) and left side (`alpha0`)
#'   are returned.
...
orientations_initial_sphericalcap_polygons <- function(betamax, n, resolution = 5*pi/180, add = NULL, full = TRUE){
  #create all grid cells
  do <- orientations_initial_sphericalcap_discretise(betamax, n)
  ...
  dpoly <- do.call(
    'rbind',
    apply(
      do[,c('band','cell','alpha0','alpha1','beta0','beta1')],
      1,
  ...
  if (min(do$ncell) == 1){
    beta1 <- do$beta1[(do$band==1)]
    dpoly <- dpoly[!((dpoly$band==1) & (dpoly$beta<beta1)), ]
  }
  #return
  return(dpoly)
}
