...
plotly_orientations_3D <- function(do, ndimension, betamax, alphaoffset = 0, betaoffset = 0, dgrid = NULL, nsignif = 3){
  ## CREATE EMPTY PLOTLY OBJECT
  p <- plotly::plot_ly()
  ## ADD ORIENTATIONS AS POINTS AND LINES
  dpp <- polar2cartesian(data = do)
  dpp$HoverText <- with(dpp, paste(
    'Azimuth: ', signif(alpha*180/pi, nsignif), ' [deg]',
    '<br>', 'Elevation: ', signif(beta*180/pi, nsignif), ' [deg]',
  ...
  ## PLOT SPHERE
  ds <- cappedsphere_vertices(beta = c(0, pi))
  p <- plotly::add_trace(
    p,
    x = ds$coordinates$x,
    y = ds$coordinates$y,
  ...
  if (ndimension == 2){
    ...
    #3D --> spherical cap
    db <- cappedsphere_vertices(beta = c(0, betamax), alphaoffset = alphaoffset, betaoffset = betaoffset)
    #add to plotly - top half of plot
